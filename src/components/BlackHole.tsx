  ...
  useEffect(() => {
    ...
    const diskMaterial = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          float spiral=vAngle*3.-(1./(normalizedRadius+0.1))*2.;
          vec2 noiseUv=vec2(vUv.x+uTime*uFlowSpeed*(2./(vRadius*0.3+1.))+sin(spiral)*0.1,vUv.y*0.8+cos(spiral)*0.1);
          float n1=snoise(vec3(noiseUv*uNoiseScale,uTime*0.15));
          float n2=snoise(vec3(noiseUv*uNoiseScale*3.+0.8,uTime*0.22));
          float n3=snoise(vec3(noiseUv*uNoiseScale*6.+1.5,uTime*0.3));
          float noiseVal=(n1*0.45+n2*0.35+n3*0.2); noiseVal=(noiseVal+1.)*0.5;
          vec3 color=uColorOuter;
          color=mix(color,uColorMid3,smoothstep(0.,0.25,normalizedRadius));
          ...
          color*=(0.5+noiseVal*1.);
          float brightness=pow(1.-normalizedRadius,1.)*3.5+0.5;
          brightness*=(0.3+noiseVal*2.2);
          float pulse=sin(uTime*1.8+normalizedRadius*12.+vAngle*2.)*0.15+0.85;
          brightness*=pulse;
          float alpha=uDensity*(0.2+noiseVal*0.9);
          alpha*=smoothstep(0.,0.15,normalizedRadius);
          alpha*=(1.-smoothstep(0.85,1.,normalizedRadius));
    ...
    const onResize = () => {
      ...
      resizeTimeout = setTimeout(() => {
        ...
        lensingPass.uniforms.aspectRatio.value = mount.clientWidth / mount.clientHeight;
        renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5));
      }, 150);
    };
